...
namespace nes
{
    ...
    void olc6502::connectBus(nes::bus *bus)
    {
        cpuBus = bus;
    }
    ...
    uint8_t olc6502::read(uint16_t addr, bool s)
    {
        if(cpuBus)
            return cpuBus->read(addr);
        else
    ...
    void olc6502::write(uint16_t addr, uint8_t value)
    {
        if(cpuBus)
    ...
    {
        write(0x100 | stkp, value);   // 0x100~0x200 栈空间范围，0x100为栈底， 0x200为栈顶 
        --stkp;                       // 栈空间是向下生长的， 复位状态SP=0xFD 指向了靠近栈顶位置，压入一个值则减1
    }

    uint8_t olc6502::pullStack()
    {
        return read(0x100 | ++stkp);  // 通pushStack
    }

    /* 中断请求，在任何时候都有可能被发送，其功能大致如下
        1. 保存现场，将各个寄存器中现有的值保存到内存对应的栈区,这里只保存了PC和PS两个寄存器的信息
        2. 从中断向量表中，读取要执行的函数地址到PC并执行
    */
    void olc6502::irq(void)
    {
        if(0 == GetFlag(I)) // I状态为0 表示不屏蔽中断
        {
            pushStack((pc >> 8) & 0xff);
            pushStack(pc & 0xff);
    ...
    void olc6502::clock(void)
    {
        if(cycles == 0) // 确保上一条指令的等待时钟全部用完
        {
            opcode = read(pc);  // 从当前程序计数器位置读取一个操作码
            pc ++;
            SetFlag(U, true);   // 总是将未使用的状态标志位设置为1, 硬件规定吧 没啥可研究的
    ...
    std::map<uint16_t, std::string> olc6502::disassemble(uint16_t nStart, uint16_t nStop)
    {
        ...
        while (addr <= (uint32_t)nStop)
        {
            ...
            uint8_t opcode = read(addr, true); addr++;
